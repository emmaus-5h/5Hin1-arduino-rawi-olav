...
void testLoop() {
  ...
  if (testToestand == DRAAIEN) {
    if (millis() - testToestandStartTijd > 1000) {
      testToestandStartTijd = millis();
      testToestand = KIJKEN;
    }
    if (testToestand == KIJKEN) {
      if (afstandM < 187 || afstandL < 187 || afstandR < 187)  {
        testToestand = DRAAIEN;
      }
      else testToestand = VOORUIT;
    }
  ...
  if (testToestand == VOORUIT) {
    analogWrite(pinMotorSnelheidR, 128);
    analogWrite(pinMotorSnelheidL, 128);
